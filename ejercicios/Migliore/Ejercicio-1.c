...
void configPort(){
	LPC_PINCON->PINSEL0 &= ~(1023<<0); // pone los pines 0 a 10 del puerto 0 como GPIO
	LPC_GPIO0->FIODIR |= (1023<<0); //pone los pines 0 a 10 del puerto 0 como salida
	LPC_GPIO0->FIOMASK &= ~(1023<<0);
}

void secuencia1(){
	LPC_GPIO0->FIOCLR = (1023<<0);
	for(int i=0; i<10; i++){
