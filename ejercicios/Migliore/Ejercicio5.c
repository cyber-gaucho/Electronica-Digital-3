...
void configPorts(){
	...
	LPC_PINCON->PINMODE0 |= (-2136298838); // 10000000 10101010 10101010 10101010
	LPC_PINCON->PINMODE1 |= (42);  // 00000000 00000000 00000000 00101010
	// no Open-Drain (default)
	// P1.0 to P1.15 INPUT, P0.0 to P0.11 and P0.15 to P0.18 OUTPUT
	LPC_GPIO0->FIODIR |= (247807);//00000000 00000011 11000111 11111111
	LPC_GPIO1->FIODIR &= (0x00FF); //00000000 00000000 11111111 11111111
...
void showResult(int32_t result){
	...
	LPC_GPIO0->FIOSET = resultL | (resultH << 15); //juntar los bytes de cada result, como hago?
}
