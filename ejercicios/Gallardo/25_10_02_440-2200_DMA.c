 ...
 * Se utiliza el Timer0 y DMA para actualizar el DAC a la frecuencia deseada
...
#include "E:\Electronica-Digital-3\common\cmsis\CMSISv2p00_LPC17xx\Drivers\inc\lpc17xx_gpdma.h"
#include "E:\Electronica-Digital-3\common\cmsis\CMSISv2p00_LPC17xx\Drivers\inc\lpc17xx_timer.h"


#define N_SAMPLES 454
// volatile uint16_t sine_bank[N_SAMPLES]; // valores ya calculados
...
void updateADCAverage(void) {
    uint16_t newVal = (LPC_ADC->ADDR0 >> 4) & 0xFFF;
    adcSum -= adcBuffer[adcIdx];
    adcBuffer[adcIdx] = newVal;
    adcSum += newVal;
...
void configADC(void) {
    PINSEL_CFG_Type PinCfg;
    PinCfg.Funcnum = 1; PinCfg.Pinnum = 23; PinCfg.Portnum = 0;
    PINSEL_ConfigPin(&PinCfg);
...
void configTimer0_DMA(void) {
    ...
    matchCfg.MatchValue = 100; // inicia 1us
    TIM_ConfigMatch(LPC_TIM0, &matchCfg);
    TIM_Cmd(LPC_TIM0, ENABLE);

    // Configurar DMA canal 0
    GPDMA_Channel_CFG_Type dmaCfg;
    dmaCfg.ChannelNum = 0;
    dmaCfg.TransferSize = N_SAMPLES;
    dmaCfg.TransferWidth = 2; // 16 bits
    dmaCfg.TransferType = GPDMA_TRANSFERTYPE_M2P; // memory -> peripheral
    dmaCfg.SrcAddr = (uint32_t)sine_bank;
    dmaCfg.DstAddr = (uint32_t)&LPC_DAC->DACR;
    dmaCfg.TransferSize = N_SAMPLES;
    dmaCfg.TransferWidth = 2;
    dmaCfg.NextBurst = 0;
    dmaCfg.TransferType = GPDMA_TRANSFERTYPE_M2P;
    dmaCfg.DMALLI = 0;
    dmaCfg.DestConn = 0; // DAC
    dmaCfg.SourceConn = 0;
    dmaCfg.DestConn = GPDMA_CONN_DAC;
    dmaCfg.TransferSize = N_SAMPLES;
    GPDMA_Setup(&dmaCfg);
    GPDMA_ChannelCmd(0, ENABLE);
}

// ---------------- Timer1 -> actualizar delayTicks según ADC ----------------
void TIMER1_IRQHandler(void) {
    if(LPC_TIM1->IR & 1) {
        LPC_TIM1->IR = 1;
        updateADCAverage();
        delayTicks = 500 - ((adcAvg*400)/4095);
        if(delayTicks<100) delayTicks=100;
    }
}
...
void configTimer1(void) {
    ...
    matchCfg.ResetOnMatch=ENABLE;
    matchCfg.ExtMatchOutputType=TIM_EXTMATCH_NOTHING;
    matchCfg.MatchValue=50000; // ej. 0.5 ms
...
// ---------------- Main ----------------
int main(void) {
    // Inicializar DAC, ADC, Timer0+DMA y Timer1
    configDAC();
    configADC();
    configTimer0_DMA();
    configTimer1();

    // // Llenar buffer DAC con onda precalculada (ejemplo)
    // for(int i=0;i<N_SAMPLES;i++)
    //     sine_bank[i]=(uint16_t)((sinf(2*3.14159*i/N_SAMPLES)+1)*512);

    // Inicializar buffer promedio ADC
    for(int i=0;i<ADC_BUFFER;i++) adcBuffer[i]=0;
    adcSum=0; adcIdx=0;

    while(1) {
        // CPU libre: DAC se genera por DMA
    }
}
